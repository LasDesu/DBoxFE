...
XMLPreferences::XMLPreferences(const QString& name, const QString& company)
{
	...
	if (name.isEmpty())
	{
		...
		else mName = new QString(myself);
	}
	else mName = new QString(name);

	if (company.isEmpty())
		mCompany = 0;
	else mCompany = new QString(company);
	...
	while (!node.isNull())
	{
		...
		if ((el.tagName() == "section") && el.hasChildNodes())
		{
			...
			else
			{
				...
				else mRoot->insert(sName, itr.value());
			}
		}
...
bool XMLPreferences::save(QFile* file)
{
	...
	if (mCompany != 0) attrs->insert("company", *mCompany);
	if (mVersionMajor > 0)
	...
	if (section.isEmpty())
	{
		...
		else
		{
			if (itr.value().data.type() != QVariant::String)
				return QString();
			else return itr.value().data.toString();
		}
	}
	else
	{
	...
	if (section.isEmpty())
	{
		...
		else
		{
			if (itr.value().data.type() != QVariant::StringList)
				return QStringList();
			else return itr.value().data.toStringList();
		}
	}
	else
	{
...
bool XMLPreferences::getBool(const QString& section, const QString& name, bool* ok)
{
	...
	if (section.isEmpty())
	{
		...
		else
		{
			...
			else return itr.value().data.toBool();
		}
	}
	else
	{
...
int XMLPreferences::getInt(const QString& section, const QString& name, bool* ok)
{
	...
	if (section.isEmpty())
	{
		...
		else
		{
			...
			else return itr.value().data.toInt();
		}
	}
	else
	{
...
qint64 XMLPreferences::getInt64(const QString& section, const QString& name, bool* ok)
{
	...
	if (section.isEmpty())
	{
		...
		else
		{
			...
			{ if (ok != 0) *ok = false; return Q_INT64_C(0); }
			else return (qint64) itr.value().data.toLongLong();
		}
	}
	else
	{
	...
	if (section.isEmpty())
	{
		...
		else
		{
			if (itr.value().data.type() != QVariant::BitArray)
				return QBitArray();
			else return itr.value().data.toBitArray();
		}
	}
	else
	{
	...
	if (section.isEmpty())
	{
		...
		else
		{
			if (itr.value().data.type() != QVariant::ByteArray)
				return QByteArray();
			else return itr.value().data.toByteArray();
		}
	}
	else
	{
...
QRect XMLPreferences::getRect(const QString& section, const QString& name)
{
	...
	if (section.isEmpty())
	{
		...
		else
		{
			...
			{ return QRect(); }
			else return itr.value().data.toRect();
		}
	}
	else
	{
...
QPoint XMLPreferences::getPoint(const QString& section, const QString& name)
{
	...
	if (section.isEmpty())
	{
		...
		else
		{
			...
			{ return QPoint(); }
			else return itr.value().data.toPoint();
		}
	}
	else
	{
...
QSize XMLPreferences::getSize(const QString& section, const QString& name)
{
	...
	if (section.isEmpty())
	{
		...
		else
		{
			...
			{ return QSize(); }
			else return itr.value().data.toSize();
		}
	}
	else
	{
	...
	if (section.isEmpty())
	{
		...
		else
		{
			if (itr.value().data.type() != QVariant::Color)
				return QColor();
			else return itr.value().data.value<QColor>();
		}
	}
	else
	{
...
bool XMLPreferences::setString(const QString& section, const QString& name, const QString& value)
{
	...
	else
	{
		...
		if (itr == mRoot->end())
		{
			if (!mAutoAddSections) return false;
			QMap<QString,EncVariant> map;
...
void XMLPreferences::parseSection(const QDomElement& rootEl, QMap<QString,EncVariant>& map)
{
	...
	while (!node.isNull())
	{
		...
		else if (type == "qbitarray")
		{
			...
			if (!ok) { node = node.nextSibling(); continue; }
