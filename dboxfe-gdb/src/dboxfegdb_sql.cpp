...
bool GameDatabaseSql::importDosBoxGameList( const QMap< QString, QMap< QString, QString> > &list, QProgressBar *pBar, QLabel *lbl )
{
	...
	while( gameIt != gameDosBoxList.end() ) {
		...
		{
			_title = gameIt.key();
			query.exec( "SELECT D.ID FROM DOSBOX D JOIN DOSBOXINFO DI ON D.ID = DI.ID WHERE DI.TITLE = '" + _title + "';" );
			if( query.isActive() )
			{
				QMap< QString, QString>::const_iterator valueIt = gameIt.value().begin();
				...
				QString id = query.value( 0 ).toString(); // DOSBox ID
...
int GameDatabaseSql::dataRowCount()
{
	if( !isOpen() )
		return 0;

	QSqlQuery query( gamedb );
	query.exec( "SELECT COUNT(*) FROM DOSBOX" );
	if( query.isActive() )
